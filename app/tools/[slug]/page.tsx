...
export async function generateMetadata({
  ...
  if (doc) {
    return {
      title: `${doc.title} - Tools`,
      description: doc.description || `${doc.title} — tool by MoctarDatt.com`,
    }
  }
  ...
  try {
    ...
    if (rows.length > 0) {
      return {
        title: `${rows[0].name} - Tools`,
        description: rows[0].description || `${rows[0].name} — tool by MoctarDatt.com`,
      }
    }
  } catch {}

  return { title: 'Tool - MoctarDatt.com' }
}
