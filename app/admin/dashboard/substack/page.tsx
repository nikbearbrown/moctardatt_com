...
export default function SubstackAdminPage() {
  ...
  function openEditSection(s: Section) {
    setEditingSection(s)
    setSectionForm({ title: s.title, slug: s.slug, description: s.description || '', substack_url: s.substack_url })
    setSectionDialogOpen(true)
  }
  ...
  async function deleteSection(id: string) {
    ...
    try {
      const res = await fetch(`/api/admin/substack/sections/${id}`, { method: 'DELETE' })
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const res = await fetch('/api/admin/substack/upload', { method: 'POST', body: formData })
      const data = await res.json()
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage your Substack newsletter sections and import articles</p>
        </div>
            ...
            <Card key={s.id}>
              <CardHeader className="flex flex-row items-start justify-between space-y-0">
                <div className="space-y-1">
                  ...
                  {s.description && <p className="text-sm text-muted-foreground pt-1">{s.description}</p>}
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => openUpload(s.id)} className="gap-1">
                    <Upload className="h-3.5 w-3.5" />
                    Import ZIP
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => openEditSection(s)}>
                    <Pencil className="h-3.5 w-3.5" />
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => deleteSection(s.id)}>
                    <Trash2 className="h-3.5 w-3.5" />
                  </Button>
                </div>
              </CardHeader>
              <CardContent>
                <a href={s.substack_url} target="_blank" rel="noopener noreferrer" className="text-sm text-muted-foreground underline">
                  {s.substack_url}
                </a>
              </CardContent>
            </Card>
      ...
      <Dialog open={sectionDialogOpen} onOpenChange={setSectionDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingSection ? 'Update the section details.' : 'Create a new Substack section.'}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  setSectionForm((prev) => ({ ...prev, title, slug: editingSection ? prev.slug : slugify(title) }))
                }}
            ...
            <div className="space-y-2">
              ...
              <Input id="section-slug" value={sectionForm.slug} onChange={(e) => setSectionForm((prev) => ({ ...prev, slug: e.target.value }))} placeholder="ai-and-writing" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="section-url">Substack URL</Label>
              <Input id="section-url" value={sectionForm.substack_url} onChange={(e) => setSectionForm((prev) => ({ ...prev, substack_url: e.target.value }))} placeholder="https://example.substack.com" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="section-desc">Description</Label>
              <Textarea id="section-desc" value={sectionForm.description} onChange={(e) => setSectionForm((prev) => ({ ...prev, description: e.target.value }))} placeholder="Brief description of this newsletter section" rows={3} />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Upload a Substack export ZIP file containing posts.csv and HTML files.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div
            ...
            >
              ...
              <p className="text-sm text-muted-foreground">Click to select a ZIP file or drag it here</p>
              <input
                ref={fileRef}
                type="file"
                accept=".zip"
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpload} disabled={uploading}>{uploading ? 'Importing…' : 'Upload & Import'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
