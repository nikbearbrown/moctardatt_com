...
const DEFAULT_BYLINE = `\u00a9 2026 Moctar Datt. All rights reserved.\n\nMoctarDatt.com`
...
export default function BlogEditor({ post, apiBase = '/api/admin/blog', redirectTo = '/admin/dashboard/blog' }: { post?: BlogPost; apiBase?: string; redirectTo?: string }) {
  ...
  async function save(publish: boolean) {
    ...
    try {
      const url = post?.id ? `${apiBase}/${post.id}` : apiBase
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="space-y-1">
        ...
        <textarea
          ...
          className="w-full text-sm border rounded-md p-3 bg-background resize-y focus:outline-none focus:ring-1 focus:ring-ring"
        />
        <p className="text-xs text-muted-foreground">Appears in pitch letters and submission cover notes.</p>
      </div>

      {/* Seed Summary */}
      <div className="space-y-1 rounded-md bg-muted/40 p-3 border border-dashed">
        <Label className="text-xs text-muted-foreground">Seed summary</Label>
        <textarea
          value={seedSummary}
          onChange={(e) => setSeedSummary(e.target.value)}
          placeholder="The core provocation — written as a directive to a writer, not a description of the article."
          rows={3}
          className="w-full text-sm border rounded-md p-3 bg-background resize-y focus:outline-none focus:ring-1 focus:ring-ring"
        />
        <p className="text-xs text-muted-foreground">Used by the rewriter. Readers never see this field.</p>
      </div>
